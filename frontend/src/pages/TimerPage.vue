<template>
  <div class="timer-page">
    ...
    <div class="colleagues-block">
      <h2>Статус колег</h2>
      <!-- Передаем обогащенный список пользователей (с groupName) -->
...
export default {
  ...
  data() {
    return {
      ...
      colleagues: [],         // список коллег (с данными о группе)
  ...
  methods: {
    ...
    // Загружаем логи, пользователей и группы, затем вычисляем статусы
    async fetchStatuses() {
      try {
        // 1) Логи
        ...
        const groups = respGroups.data.data || respGroups.data || [];
        ...
        for (const uid of Object.keys(logsByUser)) {
          ...
          const userObj = userMap.find(u => Number(u.id) === Number(uid));
          if (userObj) {
        ...
        // Обогащаем каждого пользователя названием группы
        userMap.forEach(user => {
          const grp = groups.find(g => Number(g.id) === Number(user.group_id));
          user.groupName = grp ? grp.group_name : "N/A";
        });
        ...
        if (this.currentUserId) {
          const meObj = userMap.find(u => Number(u.id) === Number(this.currentUserId));
          if (meObj) {
    ...
    parseUserLogsForToday(userLogs) {
      ...
      return {
        ...
        startTime: (finalStatus === "працює" || finalStatus === "перерва") ? lastEventStart : null,
        dayClosed,
        lastStartWork,
        lastStopWork,
        lastStartBreak,
