<template>
  <form @submit.prevent="submitForm" class="space-y-4">
    ...
    <div class="flex justify-end space-x-2 mt-4">
      <BaseButton :disabled="isSubmitting" type="submit" variant="secondary">Save</BaseButton>
...
export default {
  ...
  data() {
    return {
      ...
      localActedAt: "",
      errors: {},
      isSubmitting: false, // Флаг для предотвращения повторных отправок
    };
  },
  watch: {
    formData: {
  ...
  methods: {
    updateLocalActedAt(val) {
      ...
      const date = new Date(val * 1000); // Конвертуємо Unix-таймстемп у Date, множимо на 1000 для мілісекунд
    ...
    submitForm() {
      ...
      console.log("Submitting time log at:", new Date().toISOString(), "Data:", this.formData);
      this.errors = {};
      ...
      if (!this.localActedAt) {
        ...
        if (!isNaN(d.getTime())) {
          // Конвертуємо дату в Unix-таймстемп для бекенду
          this.formData.acted_at = Math.floor(d.getTime() / 1000); // Ділимо на 1000, щоб отримати секунди
        } else {
      ...
      if (Object.keys(this.errors).length === 0) {
        this.$emit("submit", { ...this.formData }); // Копіюємо дані, щоб уникнути мутації
      }
    ...
    resetForm() {
      ...
      this.errors = {};
      this.localActedAt = "";
      this.formData = { id: null, user_id: "", event_id: "", acted_at: "" }; // Скидаємо локально
    },
  },
};
</script>
