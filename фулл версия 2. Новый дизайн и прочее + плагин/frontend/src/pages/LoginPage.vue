...
import fetchPlugins from "@/fetchPlugins.js"; // <-- импорт вашей функции
  ...
  methods: {
    async performLogin() {
      try {
        ...
        // Шаг 3: Устанавливаем в store, что пользователь авторизован
        store.commit("setUser", { role: null, isAuthenticated: true });

        // Шаг 4: Получаем данные о пользователе (в т.ч. role_id)
        ...
        // Шаг 5: Подгружаем плагины
        // (особенно актуально, если у вас есть плагины только для админа)
        const loadedPlugins = await fetchPlugins();
        // Если у вас есть логика добавления маршрутов/меню, она должна
        // обработать "loadedPlugins" (добавить их в router, меню и т.д.)

        // Шаг 6: Редиректим пользователя
        if (userRole === "admin") {
